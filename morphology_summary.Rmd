...
#dir = "/Users/allegrasteenson/Desktop/ocotillo_research/"
setwd(dir)
...
Analyses below are multivariate models of Fouquieria splendens morphology for ocotillos located on both a bajada and a plain in Organ Pipe National Monument, Arizona. All models were grouped by their response variable and ordered by their ascending AIC values. Dataset "ocos" indicates all individuals measured on the bajada and plain while "ocos_baj" indicates only the individuals measured on the bajada. Ocotillos located on the bajada were encoded with site = 0 while ocotillos on the plain were encoded with site = 1. Competitor type is split between two types – shrub and nonshrub – where nonshrub = 0 and shrub = 1. 
...
cbind(c(str(m5$call), str(m7$call), str(m4$call), str(m1$call), str(m3$call)), summary_table)
```
...
colnames(summary_table) = c("AIC","dAIC", "Likelihood","Weight")
cbind(c(str(m2$call), str(m3$call), str(m4$call), str(m1$call), str(m0$call)), summary_table)
```
...
colnames(summary_table) = c("AIC","dAIC", "Likelihood","Weight")
cbind(c(str(m1$call), str(m0$call), str(m2$call)), summary_table)
```
...
```{r}
summary_table = round(cbind(c(AIC(m1), AIC(m0)),
                            c(delta1, delta2), c(me1, me2), 
                            c(weight1, weight2)),3)
colnames(summary_table) = c("AIC","dAIC", "Likelihood","Weight")
cbind(c(str(m1$call), str(m0$call)), summary_table)
```
...
colnames(summary_table) = c("AIC", "dAIC", "Likelihood","Weight")
cbind(c(str(m13$call), str(m7$call), str(m6$call), str(m5$call)),
      summary_table)
```
...
```{r}
summary_table = round(cbind(c(AIC(m2), AIC(m1), AIC(m5), AIC(m0), AIC(m3)),
                            c(delta1, delta2, delta3, delta4, delta5), c(me1, me2, me3, me4, me5), 
                            c(weight1, weight2, weight3, weight4, weight5)),3)

colnames(summary_table) = c("AIC", "dAIC", "Likelihood","Weight")
cbind(c(str(m2$call), str(m1$call), str(m5$call), str(m0$call), str(m3$call)), summary_table)
```
...
```{r}
summary_table = round(cbind(c(AIC(m4), AIC(m1), AIC(m5), AIC(m0)),
                            c(delta1, delta2, delta3, delta4), 
                            c(me1, me2, me3, me4), 
                            c(weight1, weight2, weight3, weight4)),3)

colnames(summary_table) = c("AIC", "dAIC", "Likelihood","Weight")
cbind(c(str(formula(m4)), str(formula(m1)), str(formula(m5)), str(formula(m0))), summary_table)
```
