...
```{r}
tidy_regression(m5, output_col) # missed this regression...
tidy_regression(m7, output_col) # also miss wrote the AIC value for this one
tidy_regression(m4, output_col)
tidy_regression(m1, output_col)
tidy_regression(m3, output_col)
```
...
```{r}
anova(m0, m2, test="Chisq") # Adding A does improve fit
...
m3 = lm(logCirc ~ X1m_Num4, data=ocos) # AB: although significant is this a meaningful number?
...
# top models are m13, m10, m7, and m6
```
