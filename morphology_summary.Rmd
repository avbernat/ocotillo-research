...
seg_baj = segs[1:1000,]                             # only ocotillos across the bajada
```
...
source("src/pretty_reg.R") # rename_regformulaME
```
...
equations = c(eq1, eq2, eq3, eq4)
equations = rename_regformula(equations, "logNumBranch", "Circ", "Inter_Plant_Group", "Inter_Distance", "Arroyo")
...
```{r}
summary_tableNN.B = gsub("Terminal_SegLengthIQRranchLengthIQR", "BranchLengthIQR", summary_tableNN.B)
```
...
m2 = lmer(Length_c ~ cbind(T1m_NumShrub, T1m_NumCacti) + (1|Tree) + (1|seg_num), data=segs)
#m2 = lmer(Length_c ~ R1m_shrub2cactus + (1|Tree) + (1|seg_num), data=segs) # not meaningful
...
equations = c(eq1, eq2, eq3, eq4)
equations = rename_regformulaME(equations, "Length", "1m_NumCacti", "Number_Nodes", "BranchLengthIQR", "tree", "seg_num")
...
```{r}
summary_tableS = gsub("1m_NumBranchLengthIQLength_cacti", "1m_NumCacti", summary_tableS)
summary_tableS = gsub("BranchLengthIQLength", "BranchLengthIQR", summary_tableS)
```
...
eqS <- paste0("Length = ", cf[1],
             ifelse(sign(cf[2])==1, " + ", " - "), abs(cf[2]), " 1m_NumCacti",
             ifelse(sign(cf[3])==1, " + ", " - "), abs(cf[3]), " Number_Nodes", 
             ifelse(sign(cf[4])==1, " + ", " - "), abs(cf[4]), " BranchLengthIQR",
             ifelse(sign(cf[5])==1, " + ", " - "), abs(cf[5]), " 1m_NumCacti*Number_Nodes",
             " + (1 | tree) + (1 | seg_num)")

commentS = "Number_Nodes, BranchLengthIQR, 1m_NumCacti*Number_Nodes"
...
Only significant covariates are listed below (some are marginal in this case):

```{r echo=FALSE}
m1 = lmer(Length_c ~ T1m_NumCacti + (1|Tree) + (1|seg_num), data=seg_baj) 
m2 = lmer(Length_c ~ cbind(T1m_NumShrub, T1m_NumCacti) + (1|Tree) + (1|seg_num), data=seg_baj) 
...
delta1 = 0 # abs(AIC(m4) - AIC(m4))
delta2 = abs(AIC(m5) - AIC(m6))
delta3 = abs(AIC(m5) - AIC(m3))
delta4 = abs(AIC(m5) - AIC(m4))
...
```{r}
summary_tableS.B = round(cbind(c(AIC(m5), AIC(m6), AIC(m3), AIC(m4)),
                            c(delta1, delta2, delta3, delta4), 
                            c(me1, me2, me3, me4), 
                            c(weight1, weight2, weight3, weight4)),3)

eq1 = c(paste(formula(m5))[2], paste(formula(m5))[1], paste(formula(m5))[3]) %>%
 paste(collapse=' ')
eq2 = c(paste(formula(m6))[2], paste(formula(m6))[1], paste(formula(m6))[3]) %>%
 paste(collapse=' ')
...
equations = c(eq1, eq2, eq3, eq4)
equations = rename_regformulaME(equations, "Length", "1m_NumCacti", "BranchLengthIQR", "Number_Nodes", "tree", "seg_num")
...
summary_tableS.B = gsub("1m_NumNumber_Nodesacti", "1m_NumCacti", summary_tableS.B)
```
...
```{r}
# Best fit model summary
cf <- round(summary(m5)$coefficient, 2)
...
write.csv(best_fit_summary,'best_fits_morph2.csv', row.names=FALSE)
```
...
write.csv(full_table,'morph_regressions2.csv', row.names=FALSE)
```
