...
par(mfrow=c(1,2))
hist(segs$Length, main="", xlab= "Terminal Segment Length (cm)", cex.lab=1.6, cex.axis=1.4, xlim=c(0,80), ylim=c(0,500))
title("(a)", adj = 0.05, line = 0, cex.main=1.5)
```


```{r}
kruskal.test(Length ~ COMP_plant_type, data = segs)
table <- pairwise.wilcox.test(segs$Length, segs$COMP_id,
                 p.adjust.method = "BH")

model1 <- kruskal.test(Length ~ COMP_plant_type, data = segs)
model2 <- kruskal.test(Length ~ COMP_id, data = segs)

#table$p.value

summary(aov(segs$Length ~ segs$COMP_id))
competitor <- segs$COMP_id
...
data = data.frame(Segment_Length=segs$Length, Competitor_Species =segs$COMP_id, COMP_plant_type = segs$COMP_plant_type)
pwc <- segs %>% 
  dunn_test(Length ~ COMP_id, p.adjust.method = "bonferroni")
pwc <- pwc %>% add_xy_position(x = "group")

res.kruskal <- segs %>% kruskal_test(Length ~ COMP_id)
res.kruskal

ggplot(segs, aes(COMP_id,Length)) + 
  geom_violin() +
    labs(
...
res.kruskal <- segs %>% kruskal_test(Length ~ COMP_plant_type)
res.kruskal

ggplot(segs, aes(COMP_plant_type,Length)) + 
  geom_violin() +
    labs(
    subtitle = get_test_label(res.kruskal, detailed = TRUE)
    ) + 
...
**By Competitor Type Comparing Bajada v.s. Plain**
...
res.kruskal <- seg_baj %>% kruskal_test(Length ~ COMP_plant_type)
res.kruskal

ggplot(seg_baj, aes(COMP_plant_type,Length)) + 
  geom_violin() +
    labs(
    subtitle = get_test_label(res.kruskal, detailed = TRUE)
    ) + 
...
#### Plain
seg_plain = seg_organ[seg_organ$site == "plain",]

pwc <- seg_plain %>% 
  dunn_test(Length ~ COMP_plant_type, p.adjust.method = "bonferroni")
pwc <- pwc %>% add_xy_position(x = "COMP_plant_type")

res.kruskal <- seg_plain %>% kruskal_test(Length ~ COMP_plant_type)
res.kruskal

ggplot(seg_plain, aes(COMP_plant_type,Length)) + 
  geom_violin() +
    labs(
    subtitle = get_test_label(res.kruskal, detailed = TRUE)
    ) + 
...
```{r}
seg_organ$site_b = seg_organ$site
seg_organ$site[seg_organ$site_b == 0] <- "bajada"
seg_organ$site[seg_organ$site_b == 1] <- "plain"
```


```{r fig.width=4, fig.height=3.5}
seg_organ$site_b = as.factor(seg_organ$site)
...
ggplot(seg_organ, aes(site,Length)) + 
  geom_violin() +
    labs(
    subtitle = get_test_label(res.kruskal, detailed = TRUE)
    ) + 
